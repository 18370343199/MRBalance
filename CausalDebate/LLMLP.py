...
from util import parse_single_choice, most_frequent, is_equiv, extract_math_answer
...
class LLMLP:
    ...
    def __init__(self, default_model_name, agents=4, agent_roles=[],prompt_tuning=[],
                 rounds=2, activation="listwise", qtype="single_choice", mtype="gpt-3.5-turbo",models=[]):
    ...
    def init_nn(self, activation, agent_roles):
        ...
        for idx in range(self.agents):
            self.nodes.append(LLMNeuron(agent_roles[idx], self.models[idx], self.ans_parser, self.qtype,prompt_tuning=self.prompt_tuning[idx]))
    ...
    def forward(self, question,logger,type="intra"):
        def get_completions():
            # get completions
        ...
        random_prompt = random.sample(self.prompt_tuning,len(self.prompt_tuning))
        activated_indices = []
        for idx, node_idx in enumerate(loop_indices):
            print(0, idx)
            self.nodes[node_idx].activate(question,logger)
            resp_cnt += 1
